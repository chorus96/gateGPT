...
실시간 지연을 시뮬레이션 가능한 사이클 수로 줄입니다. 흐름: 리셋 해제 → MMCM lock 대기 →
...
flowchart TB
    ...
    subgraph DUT["xupv5_microgpt_top #(.CLK_HZ(12.5M))"]
        direction TB
        MMCM["MMCM stub"] --> CORE["로터리→코어→LCD→미터"]
    end
    STUBS[["sim/xilinx_stubs.sv<br/>BUFG/MMCME4_BASE"]] -.-> DUT

    START(["initial 시작"]) --> RST["리셋 펄스 해제"]
    RST --> WLOCK["wait(dut.mmcm_locked)"]
...
| `clk` | `always #5`로 생성(패스스루 MMCM → 코어 클럭) |
...
- **계층 참조(XMR)**: `dut.mmcm_locked`, `dut.u_rot.armed`, `dut.u_rot.speed_level`,
...
통합 동작(MMCM lock, LCD 구동, 코어 자기생성)함을 검증. `make -C sim tb_top`으로 실행하며,
