...
`xilinx_stubs.sv`는 `xupv5_microgpt_top`이 사용하는 Xilinx **7-시리즈** 클럭 프리미티브
(`BUFG`, `MMCME2_BASE`)의 **동작 모델(behavioral stub)**입니다. Verilator/iverilog에는 Xilinx
UNISIM 라이브러리가 없으므로, 이 파일로 보드 최상위를 UNISIM 없이 시뮬레이션할 수 있습니다.

> **시뮬레이션 전용** — Vivado 프로젝트에는 포함하지 마세요. 합성에서는 실제 UNISIM 프리미티브를 사용합니다.

모델은 TOP의 필요에 맞춰 사이클 정확합니다: `BUFG`는 입력을 통과시키고, `MMCME2_BASE`는 `CLKIN1`을
`CLKOUT0`/`CLKFBOUT`으로 그대로 전달(사이클 기반 시뮬레이션에서는 x8/10 주파수 비가 무의미 — 전
설계가 단일 테스트벤치 클럭으로 동작)하며, `RST` 해제 몇 사이클 뒤 `LOCKED`를 어서트합니다.

> 원래 설계는 Virtex-5 `DCM_BASE`(CLKFX ×4/5)를 썼으나, Vivado가 Virtex-5를 지원하지 않아
> 7-시리즈 `MMCME2_BASE`(100→80 MHz)로 리타깃되었습니다.
...
- **주파수 비 무시**: 실제 MMCM은 CLKOUT0 = CLKIN1 × 8/10(80 MHz)이지만, 사이클 기반 시뮬레이션에서는
  모든 동기 요소가 단일 클럭으로 동작하므로 통과가 정확함.
- **비동기 리셋 lock**: `RST` 상승에서 즉시 unlock, 해제 후 카운터로 재lock.

## RTL과의 관계
`tb_top.sv`가 `xupv5_microgpt_top`과 함께 컴파일하여 TOP의 `MMCME2_BASE`/`BUFG` 인스턴스를 해석.
TOP의 클럭 트리(100 MHz → MMCME2 → 코어 클럭)와 `mmcm_locked` 게이트가 시뮬레이션에서 동작하도록 함.
